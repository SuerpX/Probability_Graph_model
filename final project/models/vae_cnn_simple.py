...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.decoder_l2 = nn.Sequential(
            torch.nn.Linear(256, 128 * GAME_BOARD_SIZE),
            nn.ReLU())
        
        self.decoder_l3 = nn.Sequential(
            nn.ConvTranspose2d(128, 128, kernel_size=2, stride=1),
            nn.ReLU()
            )

        self.decoder_l4 = nn.Sequential(
            nn.ConvTranspose2d(128, self.output_dim, kernel_size=2, stride=1)
            )
    def decode(self, z):
...
class VAE_CNN_Simple(nn.Module):
    def __init__(self):
        super(VAE_CNN_Simple, self).__init__()
